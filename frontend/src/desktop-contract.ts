...
export type WorkspaceModel = { snapshot?: Snapshot; polling?: PollingSnapshot; lighting?: LightingSnapshot; remap?: RemapSnapshot; inventory?: Inventory; ready: boolean; notice: string; resetConfirmation: boolean; automaticApplyBusy: boolean };

export type WorkspaceActions = { selectDevice(serial: string): void; stageDPI(index: number, value: number): void; selectStage(index: number): void; stageFeature(patch: Partial<DPIConfig>): void; stagePollingRate(rate: number): void; stageLighting(value: LightingSelection): void; stageRemap(button: number, action: RemapAction): void; applyRemap(): void; discardRemap(): void; requestReset(): void; cancelReset(): void; confirmReset(): void; retryPersistence(): void; retryPollingPersistence(): void };

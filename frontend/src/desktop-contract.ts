...
export type DesktopService = { GetSnapshot(): Promise<Snapshot>; GetPollingSnapshot(): Promise<PollingSnapshot>; GetLightingSnapshot(): Promise<LightingSnapshot>; GetRemapSnapshot(): Promise<RemapSnapshot>; RefreshStatus(): Promise<Snapshot>; RefreshInventory(): Promise<Inventory>; SelectDevice(id: DeviceID): Promise<Inventory>; StageDPI(config: DPIConfig): Promise<Snapshot>; ApplyDPI(): Promise<Snapshot>; StagePollingRate(rate: number): Promise<PollingSnapshot>; ApplyPollingRate(): Promise<PollingSnapshot>; StageLighting(selection: LightingSelection): Promise<LightingSnapshot>; ApplyRemap(config: RemapConfig): Promise<RemapSnapshot>; ApplyLighting(): Promise<LightingSnapshot>; RetryRemapPersistence(): Promise<RemapSnapshot>; ResetToFactory(): Promise<ResetResult>; RetryPersistence(): Promise<Snapshot>; RetryPollingPersistence(): Promise<PollingSnapshot>; OnStatusEvent(callback: (event: StatusEvent) => void): () => void; OnConfiguration(callback: (event: ConfigurationEvent) => void): () => void; OnPollingConfiguration(callback: (event: PollingConfigurationEvent) => void): () => void; OnRemapConfiguration(callback: (event: RemapConfigurationEvent) => void): () => void };

export type WorkspaceModel = { snapshot?: Snapshot; polling?: PollingSnapshot; lighting?: LightingSnapshot; remap?: RemapSnapshot; inventory?: Inventory; ready: boolean; notice: string; resetConfirmation: boolean };

export type WorkspaceActions = { selectDevice(serial: string): void; stageDPI(index: number, value: number): void; selectStage(index: number): void; stageFeature(patch: Partial<DPIConfig>): void; applyDPI(): void; stagePollingRate(rate: number): void; applyPollingRate(): void; stageLighting(value: LightingSelection): void; stageRemap(button: number, action: RemapAction): void; applyRemap(): void; discardRemap(): void; applyLighting(): void; requestReset(): void; cancelReset(): void; confirmReset(): void; retryPersistence(): void; retryPollingPersistence(): void };
